...
export async function GET(req: NextRequest) {
  ...
  if (req.url.includes("export=pdf")) {
    ...
    let credits = await prisma.credit.findMany({
      include: { user: { select: { name: true } } },
      orderBy: { date: "desc" },
    });
    if (customer) credits = credits.filter(c => c.customer.toLowerCase() === customer.toLowerCase());

    const doc = brandedPdf(customer ? `Credit Statement — ${customer}` : "Credit Book");
    const y = addTable(doc, [["Customer", "Item", "Amount", "Date", "Status", "By"]],
      credits.map(c => [c.customer, c.item, fmt(c.amount), c.date.toISOString().slice(0, 10), c.paid ? "Paid" : "Open", c.user.name]));
    const open = credits.filter(c => !c.paid).reduce((s, c) => s + num(c.amount), 0);
    const paid = credits.filter(c => c.paid).reduce((s, c) => s + num(c.amount), 0);
    doc.setFontSize(11);
    doc.setTextColor(4, 120, 87);
    doc.text(`Outstanding: ${fmt(open)}   •   Paid: ${fmt(paid)}`, 30, y + 26);
